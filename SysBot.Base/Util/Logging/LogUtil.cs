...
public static class LogUtil
{
    ...
    // Der Port, der in der aktuellen Instanz verwendet wird
    private static int _currentPort = 0;
    
    /// <summary>
    /// Gibt den Dateinamen der Log-Datei basierend auf dem Port zurück
    /// </summary>
    ...
    /// <summary>
    /// Setzt den Port für die aktuelle Instanz und konfiguriert Log-Dateien entsprechend
    /// </summary>
    public static void SetPort(int port)
    {
        ...
        if (LogConfig.LoggingEnabled)
        {
            ...
            {
                FileName = Path.Combine(WorkingDirectory, "logs", GetLogFileName()),
                ConcurrentWrites = true,

                ArchiveEvery = FileArchivePeriod.Day,
                ArchiveNumbering = ArchiveNumberingMode.Date,
                ArchiveFileName = Path.Combine(WorkingDirectory, "logs", $"SysBotLog_Port{_currentPort}.{{#}}.txt"),
